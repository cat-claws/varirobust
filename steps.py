...
def trades_step(net, batch, batch_idx, **kw):
	...
	scores = net(inputs_)
	loss = F.cross_entropy(scores, labels, reduction = 'sum') + 6 * F.kl_div(torch.log_softmax(scores, dim=1), F.softmax(net(inputs), dim=1), reduction='batchmean') * inputs.shape[0]
